		...
		public:
			void Load(const Identifier& id, const std::string& path){
				...
				if (!ResourceLoader::LoadFromPath(*resource, path)){
					throw std::runtime_error("ResourceManager::Load failed on: " + path);
				}
			...
			Resource& Get(const Identifier& id){
				...
				if (iterator == mResourceMap.end()){
					throw std::runtime_error("ResourceManager::GetResource id not loaded: " + std::to_string(static_cast<int>(id)));
				}
